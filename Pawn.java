...
public class Pawn extends Piece {
    ...
    public Pawn(int rank, int file, boolean player, boolean officer) {
    ...
    public boolean moveCheck(Piece[][] pieceBoard, Move moves, boolean playerTurn, int[] movesCounter) {
        ...
        if (player) {
            if (moves.sfile != moves.dfile || moves.srank - moves.drank > 2 || moves.srank - moves.drank < 0) {
                return false;
            }
            if (moves.srank != 8 && moves.srank - moves.drank > 1 || moves.srank - moves.drank < 0) {
                return false;
            }
        } else if (!player) {
            if (moves.sfile != moves.dfile || moves.drank - moves.srank > 2 || moves.drank - moves.srank < 0) {
                return false;
            }
            if (moves.srank != 1 && moves.drank - moves.srank > 1 || moves.drank - moves.srank < 0) {
                return false;
            }
        }
    ...
    public boolean captureCheck(Piece[][] pieceBoard, Move moves, boolean playerTurn, int[] movesCounter) {
        ...
        if (player) {
            if (Math.abs(moves.sfile - moves.dfile) == 1 && moves.srank - moves.drank == 1) {
                // do nothing
            } else {
                return false;
            }
        } else if (!player) {
            if (Math.abs(moves.sfile - moves.dfile) == 1 && moves.drank - moves.srank == 1) {
                // do nothing
            } else {
                return false;
            }
    ...
    public boolean promotionCheck(Piece[][] pieceBoard, Move moves, boolean playerTurn, int[] movesCounter) {
        if(!moveCheck(pieceBoard, moves, playerTurn, movesCounter)){
        return false;
        }
        return true;
    }
