...
public class Knight extends Piece {
    ...
    public Knight(int rank, int file, boolean player, boolean officer) {
    ...
    public boolean moveCheck(Piece[][] pieceBoard, Move moves, boolean playerTurn, int[] movesCounter) {
        ...
        if (pieceBoard[moves.srank][moves.sfile] == null) {
            return false;
        } else if (pieceBoard[moves.srank][moves.sfile].player != playerTurn) {
            return false;
        }
        ...
        if ((Math.abs(moves.sfile - moves.dfile) == 1 && Math.abs(moves.srank - moves.drank) == 2)
