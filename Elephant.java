...
public class Elephant extends Piece{
    ...
    public Elephant(int rank, int file, boolean player, boolean officer) {
    ...
    public boolean moveCheck(Piece[][] pieceBoard, Move moves, boolean playerTurn, int[] movesCounter) {
        ...
        if ((moves.srank == moves.drank && moves.sfile != moves.dfile) || (moves.sfile == moves.dfile && moves.srank != moves.drank)) {

            // do nothing
        } else {
            return false;
        }
        // stay on your side of the board!
        if(player){
            if(moves.drank<5){
                return false;
            }
    ...
    public boolean captureCheck(Piece[][] pieceBoard, Move moves, boolean playerTurn, int[] movesCounter) {
        ...
        // #2 The destination square is not occupied by a piece of the opposing player’s
        //color.
        if (pieceBoard[moves.drank][moves.dfile] == null) {
            return false;
        }
        ...
        // #3 The piece at the source square is not able to capture the destination square
        ...
        // #4 The current player’s King is in check, and the result of the capture does
        //not remove the King from check.
        
        // #5 The move puts the current player’s King in check
        
        // #6 The move puts the opposing player’s King in check, but the appropriate
    ...
    public boolean promotionCheck(Piece[][] pieceBoard, Move moves) {
        // TODO Auto-generated method stub
        return false;
    }
    ...
    public void Move(Piece[][] piecesBoard, Move move, boolean playerTurn, int[] movesCounter, boolean[] castlingOpportunities) {
    ...
    public String getPicName() {
        if (player) {
            return "WhiteElephant.png";
        } else {
            return "BlackElephant.png";
        }
    }
