...
public class FlyingDragon extends Piece {

    public FlyingDragon(int rank, int file, boolean player, boolean officer) {
    ...
    public boolean moveCheck(Piece[][] pieceBoard, Move moves, boolean playerTurn, int[] movesCounter) {
        ...
        if (Math.abs(moves.srank-moves.drank)==Math.abs(moves.sfile-moves.dfile)&&Math.abs(moves.srank-moves.drank)==2) {
            //do nothing
        }
        else{
            return false;
    ...
    public String getPicName() {
        if (player) {
            return "WhiteFlyingDragon.png";
        } else {
            return "BlackFlyingDragon.png";
        }
    }
