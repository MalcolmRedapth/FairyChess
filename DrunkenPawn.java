...
public class DrunkenPawn extends Piece {

    public DrunkenPawn(int rank, int file, boolean player, boolean officer) {
    ...
    public boolean moveCheck(Piece[][] pieceBoard, Move moves, boolean playerTurn, int[] movesCounter) {
        ...
        if (player) {
            if ((Math.abs(moves.sfile - moves.dfile) == 1 && moves.srank - moves.drank == 0)
                    || (moves.sfile - moves.dfile == 0 && moves.srank - moves.drank == 1)) {
                // do nothing
            } else {
                return false;
            }
            ...
            if ((Math.abs(moves.sfile - moves.dfile) == 1 && moves.srank - moves.drank == 0)
                    || (moves.sfile - moves.dfile == 0 && moves.drank - moves.srank == 1)) {
                // do nothing
            } else {
                return false;
            }
    ...
    public String getPicName() {
        if (player) {
            return "WhiteDrunkenPawn.png";
        } else {
            return "BlackDrunkenPawn.png";
        }
    }
